...
, libpulseaudio, libnotify, xdg-utils, electron_28, makeDesktopItem
, nix-update-script }:
...
buildNpmPackage rec {
  ...
  postUnpack = ''
    ...
    cp -r ${srcs.discord} source/colloid
  ...
  installPhase = let
    libPath = lib.makeLibraryPath [ libpulseaudio pipewire libnotify ];

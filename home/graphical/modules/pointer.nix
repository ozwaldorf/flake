...
with lib;
let
  cfg = config.home.pointerCursorPatch;
    ...
    options = {
      ...
      gtk = {
        ...
        size = mkOption {
          ...
          default = null;
          example = 64;
          description = "Override cursor size for gtk.";
        };
      };
    };
  };
...
{
  meta.maintainers = [
    maintainers.polykernel
    maintainers.league
  ];

  options = {
    home.pointerCursorPatch = mkOption {
  ...
  config = mkIf (cfg != null) (mkMerge [
    {
      assertions = [ (hm.assertions.assertPlatform "home.pointerCursorPatch" pkgs platforms.linux) ];
      ...
      home.sessionVariables = {
        XCURSOR_PATH = mkDefault (
          "$XCURSOR_PATH\${XCURSOR_PATH:+:}" + "${config.home.profileDirectory}/share/icons"
        );
